...
public:
    ...
    virtual void OnL2FutureSnapshotRtn(const TiQuoteSnapshotFutureField* pData)
    {
        ...
        rtn->field("bid_price_2", pData->bid_price[1]);
        rtn->field("bid_price_3", pData->bid_price[2]);
        rtn->field("bid_price_4", pData->bid_price[3]);
        rtn->field("bid_price_5", pData->bid_price[4]);
        rtn->field("bid_volume_1", pData->bid_volume[0]);
        rtn->field("bid_volume_2", pData->bid_volume[1]);
        rtn->field("bid_volume_3", pData->bid_volume[2]);
        rtn->field("bid_volume_4", pData->bid_volume[3]);
        rtn->field("bid_volume_5", pData->bid_volume[4]);
        rtn->field("ask_price_1", pData->ask_price[0]);
        rtn->field("ask_price_2", pData->ask_price[1]);
        rtn->field("ask_price_3", pData->ask_price[2]);
        rtn->field("ask_price_4", pData->ask_price[3]);
        rtn->field("ask_price_5", pData->ask_price[4]);
        rtn->field("ask_volume_1", pData->ask_volume[0]);
        rtn->field("ask_volume_2", pData->ask_volume[1]);
        rtn->field("ask_volume_3", pData->ask_volume[2]);
        rtn->field("ask_volume_4", pData->ask_volume[3]);
        rtn->field("ask_volume_5", pData->ask_volume[4]);
        rtn->timestamp(pData->timestamp);
